...
def item_cf(user_id, item_id, similar_items, user_to_rating):
    ...
    # user_to_rating: 943 users x 1682 items
    for liked_item, rating in enumerate(user_to_rating[user_id]):
        if rating < 3: 
            continue
        
        liked_item += 1
        # print(f"user{user_id}, item{item_id}, liked_item{liked_item}, rating{rating}, similar_items{len(similar_items)}")
        for similar_item, similarity in similar_items[item_id]:
            if similar_item == liked_item:
                pred_score += rating * similarity
                count += 1
                break
    
    return pred_score / (count + 1e-5)
